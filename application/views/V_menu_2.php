				...
				<div class="page-content">
					...
					<div class="row" id="table">
						<table id="example" class="table table-bordered table-striped table-hovered example">
							...
							<?php
								foreach ($get_data_count as $doc) {
									$DOC_MAKER = $doc->DOC_MAKER;
								}
								if ($SESSION_ID == $DOC_MAKER || $SESSION_DEPARTEMENT_CODE == "BPI"):
									$get = $this->M_library_database->GET_DOC_ARCHIVED($SESSION_ID);
									foreach ($get as $get):
										date_default_timezone_set('Asia/Jakarta');
										$DOC_DATE_EXPIRED = date('Y-m-d',strtotime($get->DOC_TGL_EXPIRED));
							...
							?>
									<tbody>
										<tr>
											<td><?php echo $get->DOC_ID?></td>
											<td><?php echo $get->DOC_NAMA?></td>
											<td><?php echo $get->DN_NAME?></td>
											<td><?php echo date('d/m/Y',strtotime($get->DOC_DATE))?></td>
											<td><?php echo $get->DOC_STATUS?></td>
											<td><?php echo "" . $y . " tahun " . $m . " bulan " . $d . " hari";?></td>
											<td>
												<a href="<?php echo base_url('document-details-'.$get->DOC_ID) ?>" class="fa fa-eye" style="font-size: 2rem;text-decoration: none;color: black;" target="_blank"></a>
												<a style="font-size: 2rem;text-decoration: none;color: black;" class="fa fa-download" href="<?=base_url('download-'.$get->DOC_ID.".zip");?>" id="btn-unduh" class="btn btn-sm btn-warning"></a>
											</td>
										</tr>
									</tbody>
